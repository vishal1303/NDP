        ...
        for s in skew:
            ...
            for b in bandwidth:
                try:
                    ...
                    continue
            out = open(dirname+"/"+protocol+"-ANY"+"-"+d+"-"+str(l)+"-"+str(s)+".out.slowdown.all.mean", "w")
            out.write("arch ")
            for i in range(len(bandwidth)):
                out.write(str(bandwidth[i])+" ")
            out.write("\n")
            out.write(protocol)
            out.write(" ")
        ...
        for s in skew:
            ...
            for b in bandwidth:
                try:
                    ...
                    slowdown_val.append("1")
                    continue
            out = open(dirname+"/"+protocol+"-ANY"+"-"+d+"-"+str(l)+"-"+str(s)+".out.slowdown.all.99", "w")
            out.write("arch ")
            for i in range(len(bandwidth)):
                out.write(str(bandwidth[i])+" ")
            out.write("\n")
