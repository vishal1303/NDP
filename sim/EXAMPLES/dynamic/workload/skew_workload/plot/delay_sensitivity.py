        ...
        for s in skew:
            ...
            for d in delay:
                try:
                    ...
                    continue
            out = open(dirname+"/"+protocol+"-"+b+"-ANY"+"-"+str(l)+"-"+str(s)+".out.slowdown.all.mean", "w")
            out.write("arch ")
            for i in range(len(delay)):
                tokens = delay[i].split('.')
                out.write(tokens[0]+" ")
            out.write("\n")
            out.write(protocol)
            out.write(" ")
        ...
        for s in skew:
            ...
            for d in delay:
                try:
                    ...
                    slowdown_val.append("1")
                    continue
            out = open(dirname+"/"+protocol+"-"+b+"-ANY"+"-"+str(l)+"-"+str(s)+".out.slowdown.all.99", "w")
            out.write("arch ")
            for i in range(len(delay)):
                tokens = delay[i].split('.')
                out.write(tokens[0]+" ")
