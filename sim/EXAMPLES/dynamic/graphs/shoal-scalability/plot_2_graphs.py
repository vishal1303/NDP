...
def hw_histogram_from_file(keys, fnames, fig_path, fig_name, x_label, y_labels, vert_line=0, remove_points=None, y_points=None,logscale=False,hide_legend=False, ind_multiplier=1,legend_cols=2,err=None,label_replacer=None):
    ...
    for fname in fnames:
        ...
        if k==1:
            plt.ylim([0,80])
            start, end = ax.get_ylim()
            ax.yaxis.set_ticks(np.arange(start, end+1, 20))
...
keys = [('crimson', '256'), ('dodgerblue', '512'), ('orange', '768'), ('turquoise', '1024')]
label_replacer = {'256': '256 nodes', '512': '512 nodes', '768': '768 nodes', '1024': '1024 nodes'}
hw_histogram_from_file(keys, ["dc_workload.fct.99.9", "dc_workload.tput.avg"], load_path, "dc_workload_scalability", "Load", ["FCT (us)", "Throughput (Gbps)"], label_replacer=label_replacer)
#keys = [('x', 'School')]
