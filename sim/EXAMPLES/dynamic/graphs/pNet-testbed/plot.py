...
def hw_histogram_from_file(keys, fname, fig_path, fig_name, x_lable, y_label, vert_line=0, remove_points=None, y_points=None,logscale=False,hide_legend=False, ind_multiplier=1,legend_cols=2,err=None,label_replacer=None, utilization=False):
    labels = []
    key_to_list = {}
    ...
    if not hide_legend:
        plt.legend(bbox_to_anchor=(0., 0.84, 1., 0.), loc=3,
            ncol=2, fontsize=font['size']-6)
    #if not hide_legend:
...
keys = [('crimson', 'pNet')]
hw_histogram_from_file(keys, "utilization.dat", load_path, "testbed-utilization", "Workload", "Network Utilization (%)", hide_legend=True, utilization=True)
