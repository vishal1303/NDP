...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
  ...
  for key in keys:
    ...
    plt.ylim([0,100])
    plt.xlim([1,512])
    plt.axhline(90, color="k", linestyle="--", linewidth=2)
    ...
    #ax.text(70, 90+2.0, 'Maximum Achievable Throughput',size=20)
    x_ticks = []
    x_tick_pos= []
...
Keys = ["Shoal"]
plot_line_graph(load_path,Keys,["tput.mean"],"PERM_TPUT","Throughput (Gbps)",xaxis="Communicating pairs", xtransform=None)
