...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
    ...
    for key in Keys:
        ...
        if i==0:
            ...
            x = [np.percentile(data,perc) for perc in y]
            plt.plot(x,[v/100.0 for v in y],'o',color=c[i],label=key,ms=12)
        i = i + 1
    ...
    plt.rc('font', **font)
    plt.xlabel(xaxis)
    lgd = plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), fancybox=False, shadow=True, ncol=2, fontsize=font['size']-3)
    plt.savefig(graphs_path+"CDF_"+title, bbox_inches='tight')
...
Keys=["Prototype","Simulator"]
plot_line_graph(load_path,Keys,["prototype-latency.cdf","simulator-latency.cdf"],"latency","CDF",xaxis="Flow completion time (us)")
