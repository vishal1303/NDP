...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
  ...
  first_legend = plt.legend(handles=[line1],loc=2,fontsize=24)
  ...
  for key in keys:
    ...
    if i == 1:
        line3[i], = plt.plot(x,[v for v in y],'s', color='limegreen',label=key, ms=14)
    elif i == 2:
  ...
  plt.legend(handles=[line3[k] for k in range(len(keys))],loc=4,ncol=1,fontsize=24)
