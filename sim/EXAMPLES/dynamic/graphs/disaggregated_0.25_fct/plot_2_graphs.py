...
def hw_histogram_from_file(keys, fnames, fig_path, fig_name, x_label, y_labels, vert_line=0, remove_points=None, y_points=None,logscale=False,hide_legend=False, ind_multiplier=1,legend_cols=2,err=None,label_replacer=None):
    idx = 211
    k=0
    plt.figure(figsize=(14,10))
    for fname in fnames:
        plt.subplot(idx)
        plt.subplots_adjust(bottom=0,hspace=0.1)
        ax = plt.gca()
        if k==0:
            plt.ylim([0,15])
            start, end = ax.get_ylim()
            ax.yaxis.set_ticks(np.arange(start, end+1, 3))
        if k==1:
            plt.ylim([0,200])
            start, end = ax.get_ylim()
            ax.yaxis.set_ticks(np.arange(start, end+1, 30))
        ...
        for i,stype in enumerate(keys):
            ...
            #plt.bar(ind + i*width, key_to_list[stype[1]] ,width, color='white',edgecolor='black', hatch=stype[0], label=lab)
            plt.bar(ind + i*width, key_to_list[stype[1]] ,width, color=stype[0],edgecolor='black', label=lab)
        ...
        #plt.rcdefaults()
        if k == 0:
            ax.set_ylabel(y_labels[k].strip(), fontsize=font['size'], labelpad=25)
        if k == 1:
            ax.set_ylabel(y_labels[k].strip(), fontsize=font['size'])
            ax.set_xlabel(x_label.strip(), fontsize=font['size'], labelpad=1)
            lgd = ax.legend(loc='upper center', bbox_to_anchor=(0.5, -1), fancybox=False, shadow=True, ncol=legend_cols, fontsize=font['size']-3)
            plt.savefig(fig_path+fig_name,bbox_extra_artists=(lgd,), bbox_inches='tight')
        k = k + 1

#def hw_histogram_from_file(keys, fnames, fig_path, fig_name, x_lable, y_label, vert_line=0, remove_points=None, y_points=None,logscale=False,hide_legend=False, ind_multiplier=1,legend_cols=4,err=None,label_replacer=None):
#    idx = 211
#    for fname in fnames:
#        plt.subplot(idx)
#        ax = plt.gca()
...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
  plt.clf()
  plt.rcdefaults()
  line1, = plt.plot([0],label="Optimal", linestyle='--',color='b',linewidth=2)
  #line2, = plt.plot([0],label="Slowest", color='k')
  first_legend = plt.legend(handles=[line1],loc=2)
  ax = plt.gca().add_artist(first_legend)
  i = 0
  line3 = [None for j in range(len(keys))]
  color=['b']
  for key in keys:
    x = []
    y = []
    ...
    if i == 1:
        line3[i], = plt.plot(x,[v for v in y],'cs', label=key, ms=12)
    elif i == 2:
        line3[i], = plt.plot(x,[v for v in y],'ro',label=key, ms=9)
    else:
        line3[i], = plt.plot(x,[v for v in y],linestyle='--',color=color[i], linewidth=2)
    i=i+1

  plt.legend(handles=[line3[k] for k in range(len(keys))],loc=4,ncol=1)
...
#hw_histogram_from_file(keys, ["dc_workload.fct.mean", "dc_workload.fct.99.9"], load_path, "dc_workload_fct", "Load", ["Avg FCT(us)", "99.9-perc.FCT(us)"])
...
hw_histogram_from_file(keys, ["disaggregated_0.25.fct.mean","disaggregated_0.25.fct.99.9"], load_path, "disaggregated_0_25_fct", "Application", ["Avg FCT(us)","99.9-perc.FCT(us)"], label_replacer=label_replacer)
