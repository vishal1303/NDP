...
def hw_histogram_from_file(keys, fname, fig_path, fig_name, x_lable, y_label, vert_line=0, remove_points=None, y_points=None,logscale=False,hide_legend=False, ind_multiplier=1,legend_cols=2,err=None,label_replacer=None):
    ...
    ax.set_xticklabels([l.strip() for l in labels], rotation=0)
...
keys = [('crimson', 'pNet'), ('dodgerblue', 'fastpass'), ('orange', 'phost'), ('turquoise', 'pfabric')]
#('grey', '500KB-1MB'), ('yellow', '1MB-5MB'), ('magenta', '5MB-10MB'), ('black', '>10MB')]
label_replacer = {'pNet': 'pNet', 'fastpass': 'Fastpass', 'phost': 'pHost', 'pfabric': 'pFabric'}
for i in range(1,9):
    hw_histogram_from_file(keys, load_path+str(i)+".slowdown.mean", load_path, str(i)+"_slowdown_mean", "Load", "Mean slowdown", label_replacer=label_replacer)
