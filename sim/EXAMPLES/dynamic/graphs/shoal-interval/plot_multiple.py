...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
    c = ['crimson', 'blue', 'orange', 'limegreen','crimson', 'blue', 'orange', 'limegreen','crimson', 'blue', 'orange', 'limegreen']
    for graph in range(3):
        ...
        #    continue
        if (graph == 0):
            Keys = [keys[k] for k in range(2)]
            print Keys
            i = 0
        elif (graph == 1):
            Keys = []
            for k in xrange(2,4,1):
                Keys.append(keys[k])
            print Keys
            i = 2
        else:
            Keys = []
            for k in xrange(4,len(keys),1):
                Keys.append(keys[k])
            print Keys
            i = 4
...
Keys=["Uniform scheduling","Selective scheduling","Uniform scheduling","Selective scheduling","Uniform scheduling","Selective scheduling"]
plot_line_graph(load_path,Keys,["0-0.cdf","0-511.cdf","0.25-0.cdf","0.25-511.cdf","0.5-0.cdf","0.5-511.cdf"],"interval_cdf","CDF",xaxis="Flow completion time (us)")
