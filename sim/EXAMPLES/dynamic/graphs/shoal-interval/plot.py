...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
    c = ['blue', 'crimson', 'orange', 'limegreen','crimson', 'blue', 'orange', 'limegreen','crimson', 'blue', 'orange', 'limegreen']
    plt.clf()
    plt.rcdefaults()
    fig,ax = plt.subplots()
    i=0
    ...
    plt.rc('font', **font)
    plt.xlabel(xaxis)
    lgd = plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), fancybox=False, shadow=True, ncol=4, fontsize=font['size']-5, labelspacing=0, handlelength=1.5)
    plt.savefig(graphs_path+"CDF_"+title, bbox_inches='tight')
...
Keys=["Uniform scheduling","Selective scheduling"]
plot_line_graph(load_path,Keys,["0-0.cdf","0-511.cdf"],"interval_cdf","CDF",xaxis="Flow completion time (us)")
