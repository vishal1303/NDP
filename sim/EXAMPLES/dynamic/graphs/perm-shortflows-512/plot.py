...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
    c = ['orange', 'crimson', 'limegreen', 'blue', 'crimson', 'limegreen']
    IDX = 211
    for graph in range(2):
        ...
        if (graph==0):
            ...
            ax.yaxis.set_ticks(np.arange(start, end+0.1, 20))
            objects = ('Shoal','DCTCP','DCQCN')
            y_pos = np.arange(len(objects))
            width = 0.35
            performance = [31,17,18]
            dev = [[7,6,6],[3,65,65]]
            ...
            continue
        if (graph == 1):
            Keys = [keys[k] for k in range(3)]
            i = 0
        else:
            Keys = [keys[k] for k in xrange(3,len(keys),1)]
            i = 3
        for key in Keys:
            plt.ylim([0.95,1.005])
            #start, end = ax.get_ylim()
            ...
            if (graph==1):
                plt.xlim([20,90])
                #ax.text(25,0.1,"Uniform\ncross-traffic",size=22)
            else:
                plt.xlim([0,400])
                start, end = ax.get_xlim()
                ax.xaxis.set_ticks(np.arange(start, end+1, 100))
                ax.text(95,0.1,"Skewed\ncross-traffic",size=22)
            x_ticks = []
            x_tick_pos= []
            data = []
            f = open(fname[i],'r')
            ...
            f.close()
            y = [0.01*idx for idx in range(9500,10001)]
...
Keys=["Shoal","DCTCP","DCQCN"]
plot_line_graph(load_path,Keys,["../../perm-shortflows-512/Shoal.cdf","../../perm-shortflows-512/512.csv.dctcp.fct.cdf","../../perm-shortflows-512/512.csv.dcqcn.fct.cdf"],"perm_shortflows_cdf","CDF",xaxis="Flow completion time (us)")
